...
getStreak <- function(allMatches, index, prevRes, streakLength) {
  if (index < 1) {
    return(list(streakLength = streakLength, streakType = prevRes, index = index))
  }
...
getAllStreaks <- function(allMatches, index, allStreaks = tibble(streakLength = NA, streakType = "", index = NA)) {
  matchesLeft <- head(allMatches, index)
  if (index > 0 & nrow(matchesLeft) > 0) {
    streak <- getStreak(matchesLeft, nrow(matchesLeft), NULL, 0)
    return(getAllStreaks(matchesLeft, streak$index, allStreaks %>% add_row(streakLength = streak$streakLength, streakType = streak$streakType, index = index)))
  }
  return(allStreaks)
}


pblapply(leagues[5], function(l) {
  ...
  return(list(allMatches = allMatches %>% select(success), leagueName = l, streaks = allStreaks))
})
```
